...
abstract class BasePage<Controller extends BaseController>
  ...
  const BasePage({this.pageController, this.autoDispose = true, Key? key})
    : super(key: null);
}

typedef ControlledBuilder<Controller extends BaseController> =
    Widget Function(BuildContext context, Controller controller);

class ControlledWidgetBuilder<Controller extends BaseController>
    extends StatelessWidget {
  final ControlledBuilder<Controller> builder;

  const ControlledWidgetBuilder({required this.builder, super.key});
